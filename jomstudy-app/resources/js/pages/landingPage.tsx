...
const highlights = [
    {
        title: 'landing.features.ai_learning.title',
        description: 'landing.features.ai_learning.description',
        icon: Brain,
    },
    {
        title: 'landing.features.resource_sharing.title',
        description: 'landing.features.resource_sharing.description',
        icon: BookOpen,
    },
    {
        title: 'landing.features.qna_community.title',
        description: 'landing.features.qna_community.description',
        icon: MessageSquare,
    },
    {
        title: 'landing.features.achievements.title',
        description: 'landing.features.achievements.description',
        icon: Trophy,
    },
    {
        title: 'landing.features.multilingual_support.title',
        description: 'landing.features.multilingual_support.description',
        icon: Globe,
    },
    {
        title: 'landing.features.save_follow.title',
        description: 'landing.features.save_follow.description',
        icon: Bookmark,
    },
];
...
export default function LandingPage({ canRegister }: LandingPageProps) {
    ...
    return (
        <>
            <Head>
                <title>{trans('landing.meta_title')}</title>
                <meta
                    name="description"
                    head-key="description"
                    content={trans('landing.meta_description')}
                />
                <meta
                    property="og:title"
                    head-key="og:title"
                    content={trans('landing.meta_title')}
                />
                <meta
                    property="og:description"
                    head-key="og:description"
                    content={trans('landing.social_description')}
                />
                    ...
                    <section className="relative overflow-hidden border-b border-zinc-200/80 bg-[radial-gradient(circle_at_10%_20%,#ffe6df_0,#fff4eb_36%,#fffdf8_75%)]">
                        <div className="mx-auto flex w-full max-w-6xl flex-col gap-8 px-4 py-16 md:px-8 md:py-24">
                            <div className="max-w-3xl">
                                <h1 className="text-4xl leading-tight font-black tracking-tight sm:text-5xl md:text-6xl">
                                    <span className="text-[#d6597a]">
                                        {trans('landing.hero_title_long')}
                                    </span>
                                </h1>
                                <p className="mt-6 max-w-2xl text-base text-zinc-700 sm:text-lg">
                                    {trans('landing.hero_subtitle')}
                                </p>

                                <div className="mt-8 flex flex-wrap items-center gap-3">
                                    <Button
                                        asChild
                                        className="h-11 bg-[#d6597a] px-6 text-sm font-semibold hover:bg-[#c84e6f]"
                                    >
                                        <Link href={login()}>
                                            {trans('landing.start_learning')}
                                        </Link>
                                    </Button>
                                    {canRegister ? (
                                        <Button
