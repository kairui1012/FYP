...
} from '../ts/features/categories/category-config';
import type { TransFn } from '../ts/features/categories/category-types';
...
export function CategoryPostCard({
    post,
    currentUserId,
    liked,
    likesCount,
    ...
    const { bg: typeBg, text: typeText } = getPostTypeBadgeProps(
        post.post_type,
    );
    const typeLabel =
        post.post_type === 'quiz'
        ...
        <div>
            ...
            <div className="mt-2 flex items-center gap-3 px-5 text-sm text-zinc-900">
                ...
                <BtnComment
                    ...
                    className="mb-2"
                    onClick={() =>
                        router.visit(`/posts/${post.id}?focus=comments`)
                    }
                />
                <BtnSave
                    count={savesCount}
