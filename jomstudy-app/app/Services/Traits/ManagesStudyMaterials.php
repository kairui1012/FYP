...
trait ManagesStudyMaterials
{
     ...
     */
    private function buildMaterialFeedbackSummary(Post $post): array
    {
        return $this->formatMaterialFeedbackSummary(
            $this->materialVersionService->buildFeedbackSnapshot($post),
        );
    }
    ...
    private function buildLearningAnalytics(Post $post): array
    {
        ...
        $feedback = $this->buildMaterialFeedbackSummary($post);
    ...
    private function buildLinkedQuizzes(Post $post, array $followingIds, ?int $userId = null): array
    {
        ...
        $quizzes = Post::query()
            ...
            ->with(['user:id,name,role,is_verified', 'user.socialAccounts:id,user_id,avatar', 'subject:id,name', 'lesson:id,title,sequence', 'language:id,code,name'])
     ...
     */
    private function attachMaterialFeedbackSummaries(Collection $posts): void
    {
        $materialIds = $posts
            ...
            ->values()
            ->all();
        $snapshots = $this->materialVersionService->buildFeedbackSnapshots($materialIds);

        $posts->each(function (Post $post) use ($snapshots) {
            if ($post->post_type !== 'material') {
                return;
            }

            $snapshot = $snapshots[$post->id] ?? $this->materialVersionService->emptyFeedbackSnapshot();
            $post->setAttribute('material_feedback_summary', $this->formatMaterialFeedbackSummary($snapshot));
        });
    }

    /** @param array{average_rating: float, rating_count: int, recommended_count: int, not_recommended_count: int, total_votes: int, recommendation_rate: int} $snapshot */
    private function formatMaterialFeedbackSummary(array $snapshot): array
    {
        return [
            ...
            'not_recommended_count' => $snapshot['not_recommended_count'],
